...
onready var No_Preset_Warning = load("res://Preset_Warnung.tscn")
...
func input_checker():
	...
	# change the time intervall of the shuffle modes
	if Input.is_action_just_pressed("shuffle_extend"):
		if ShuffleIntervall + 1 <= 10:
			ShuffleIntervall += 1
			set_shuffle_intervall()
			$IntervallLabel.visible = true
			$IntervallLabel/SichbarkeitsTimer.start()
	if Input.is_action_just_pressed("shuffle_shorten"):
		if ShuffleIntervall - 1 >= 1:
			ShuffleIntervall -= 1
			set_shuffle_intervall()
			$IntervallLabel.visible = true
			$IntervallLabel/SichbarkeitsTimer.start()
...
func shuffle_handler():
	...
	if ShuffleMode == 2:
		if ColorShuffleArray.size() > 0:
			...
			ShuffleMode = 1
			var warning = No_Preset_Warning.instance()
			var window_size = OS.get_real_window_size()
			warning.global_position = Vector2(window_size[0] / 3, window_size[1] / 3)
			add_child(warning)

func set_shuffle_intervall():
	$ShuffleTimer.wait_time = ShuffleIntervall


func show_new_intervall():
	$IntervallLabel.text = String(ShuffleIntervall) + " Sekunden"

func _on_SichbarkeitsTimer_timeout():
